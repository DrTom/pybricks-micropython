...
// Diagnostics for incoming Pybricks commands (BaseHub USB bring-up).
volatile uint32_t pbsys_diag_command_calls;
volatile uint32_t pbsys_diag_command_last_cmd;
volatile uint32_t pbsys_diag_command_last_size;
volatile uint32_t pbsys_diag_command_last_arg1;
volatile uint32_t pbsys_diag_command_last_result;
volatile uint32_t pbsys_diag_command_count_stop;
volatile uint32_t pbsys_diag_command_count_start_user;
volatile uint32_t pbsys_diag_command_count_start_repl;
volatile uint32_t pbsys_diag_command_count_write_stdin;
volatile uint32_t pbsys_diag_command_count_other;
...
pbio_pybricks_error_t pbsys_command(const uint8_t *data, uint32_t size) {
    ...
    pbsys_diag_command_calls++;
    pbsys_diag_command_last_cmd = cmd;
    pbsys_diag_command_last_size = size;
    pbsys_diag_command_last_arg1 = size > 1 ? data[1] : 0;

    switch (cmd) {

        case PBIO_PYBRICKS_COMMAND_STOP_USER_PROGRAM:
            pbsys_diag_command_count_stop++;
            pbsys_program_stop(false);
            pbsys_diag_command_last_result = PBIO_PYBRICKS_ERROR_OK;
            return PBIO_PYBRICKS_ERROR_OK;

        case PBIO_PYBRICKS_COMMAND_START_USER_PROGRAM: {
            pbsys_diag_command_count_start_user++;
            if (size > 2) {
                pbsys_diag_command_last_result = PBIO_PYBRICKS_ERROR_VALUE_NOT_ALLOWED;
                return pbsys_diag_command_last_result;
            }
            // Use payload as program ID, otherwise use active user slot.
            pbsys_diag_command_last_result = pbio_pybricks_error_from_pbio_error(
        ...
        case PBIO_PYBRICKS_COMMAND_START_REPL:
            pbsys_diag_command_count_start_repl++;
        ...
        case PBIO_PYBRICKS_COMMAND_WRITE_USER_PROGRAM_META:
            pbsys_diag_command_count_other++;
            pbsys_diag_command_last_result = pbio_pybricks_error_from_pbio_error(pbsys_storage_set_program_size(
                pbio_get_uint32_le(&data[1])));
            return pbsys_diag_command_last_result;

        case PBIO_PYBRICKS_COMMAND_WRITE_USER_RAM:
            pbsys_diag_command_count_other++;
            pbsys_diag_command_last_result = pbio_pybricks_error_from_pbio_error(pbsys_storage_set_program_data(
        ...
        case PBIO_PYBRICKS_COMMAND_WRITE_STDIN:
            pbsys_diag_command_count_write_stdin++;
            #if PBSYS_CONFIG_HOST
            if (pbsys_host_stdin_get_free() < size - 1) {
                pbsys_diag_command_last_result = PBIO_PYBRICKS_ERROR_BUSY;
                return pbsys_diag_command_last_result;
            }
        ...
        case PBIO_PYBRICKS_COMMAND_WRITE_APP_DATA: {
            ...
            pbsys_diag_command_last_result = pbio_pybricks_error_from_pbio_error(write_app_data_callback(offset, data_size, data_to_write));
            return pbsys_diag_command_last_result;
        }
        default:
            pbsys_diag_command_count_other++;
            pbsys_diag_command_last_result = PBIO_PYBRICKS_ERROR_INVALID_COMMAND;
            return pbsys_diag_command_last_result;
    }
}
