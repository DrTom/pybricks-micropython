...
pbio_error_t pbio_port_lump_data_recv_thread(pbio_os_state_t *state, pbio_port_lump_dev_t *lump_dev, pbdrv_uart_dev_t *uart_dev) {
    ...
    while (true) {
        ...
        if (err != PBIO_SUCCESS) {
            if (err == PBIO_ERROR_TIMEDOUT) {
                // A header-read timeout at 100 Hz send rates (10 ms/packet) means
                // at least 250 ms without a single byte — the stream is likely lost.
                // Count it as a desync event; flush the UART on threshold and give
                // up so the keepalive logic can trigger a full reconnect.
            ...
            debug_pr("UART Rx data header error\n");
            return err;
        }
