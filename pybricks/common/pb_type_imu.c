...
// pybricks._common.IMU.set_samples_count_max
static mp_obj_t pb_type_imu_set_samples_count_max(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    PB_PARSE_ARGS_METHOD(n_args, pos_args, kw_args,
        pb_type_imu_obj_t, self,
        PB_ARG_REQUIRED(samples));

    (void)self;
    pbio_imu_set_samples_count_max(mp_obj_get_int(samples_in));
    return mp_const_none;
}
MP_DEFINE_CONST_FUN_OBJ_KW(pb_type_imu_set_samples_count_max_obj, 1, pb_type_imu_set_samples_count_max);
...
static const mp_rom_map_elem_t pb_type_imu_locals_dict_table[] = {
    ...
    { MP_ROM_QSTR(MP_QSTR_set_samples_count_max), MP_ROM_PTR(&pb_type_imu_set_samples_count_max_obj)},
